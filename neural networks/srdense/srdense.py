...
# imports for dense layer
from keras.layers import Conv2D, BatchNormalization, ReLU
# imports for dense block
from keras.layers import concatenate
# imports for transition block
...
def dense_block(x, layers=3, filters=12, kernel_size=(3, 3), strides=(1, 1)):
    """
...
def transition_layer(inputs, filters=12, compression=1.0, weight_decay=1e-4):
    """
...
def dense_model(x_train, blocks=[3, 4, 5], filters=12, kernel_size=(3, 3), strides=(1, 1), weight_decay=1e-4):
    if len(blocks) < 2:
        return None
    ...
    for i in range(len(blocks) - 1):
        x = dense_block(x, layers=blocks[i], filters=filters)
        x = transition_layer(x)
    x = dense_block(x, layers=blocks[-1], filters=filters)
...
def main():
    ...
    dense_model_net = dense_model(x_train, blocks=[3,4,5], filters=12)
    ...
    #dense_model_net.fit(x_train, y_train, epochs = 80, shuffle= True, validation_split=0.1)
